...
marc1 = chrono.ChLineArc(
    chrono.ChCoordsysD(chrono.ChVectorD(2, 2.5, 0)),
    0.5,
    -chrono.CH_C_PI_2,
    chrono.CH_C_PI_2,
    True,
...
marc2 = chrono.ChLineArc(
    chrono.ChCoordsysD(chrono.ChVectorD(1, 2.5, 0)),
    0.5,
    chrono.CH_C_PI_2,
    -chrono.CH_C_PI_2,
    True,
...
mtrajectory.Initialize(
    mpendulum,  # body1 that follows the trajectory
...
ms1 = chrono.ChLineSegment(
    chrono.ChVectorD(-0.5, -0.5, 0), chrono.ChVectorD(0.5, -0.5, 0)
)
...
ms2 = chrono.ChLineSegment(
    chrono.ChVectorD(0.5, 0.5, 0), chrono.ChVectorD(-0.5, 0.5, 0)
)
mglyph.AddSubLine(ms2)
ma2 = chrono.ChLineArc(
    chrono.ChCoordsysD(chrono.ChVectorD(-0.5, 0, 0)),
    0.5,
    chrono.CH_C_PI_2,
    -chrono.CH_C_PI_2,
...
# Define which parts are connected (the trajectory is considered in the 2nd body).
mglyphconstraint.Initialize(
    mpendulum2,  # body1 that follows the trajectory
    mwheel,  # body2 that 'owns' the trajectory
    True,
    chrono.ChCoordsysD(
        chrono.ChVectorD(0, 0.5, 0)
    ),  # point on body1 that will follow the trajectory
    chrono.ChCoordsysD(),
)
